...
def parse_args():
    ...
    parse.add_argument('--config', dest='config', type=str,
            default='configs/bisenet_loveda.py',)
    parse.add_argument('--finetune-from', type=str, default='./res/model_final.pth',)
...
"""def set_model(lb_ignore=255):
    ...
    if not args.finetune_from is None:
        ...
        if v1:
            ...
            # Rimuovi 'module.' se presente (multi-GPU training)
            new_state_dict = {}
            for k, v in state_dict.items():
                new_k = k.replace('module.', '') if k.startswith('module.') else k
                new_state_dict[new_k] = v

            # Filtra i layer finali non compatibili (legati al numero di classi)
            excluded_keys = [
                'head.conv_out.1', 'aux2.conv_out.1', 'aux3.conv_out.1',
                'aux4.conv_out.1', 'aux5_4.conv_out.1'
            ]
            filtered_dict = {
                k: v for k, v in new_state_dict.items()
                if not any(exclude in k for exclude in excluded_keys)
            }

            msg = net.load_state_dict(filtered_dict, strict=False)
            logger.info('\tMissing keys: ' + json.dumps(msg.missing_keys, indent=2))
            logger.info('\tUnexpected keys: ' + json.dumps(msg.unexpected_keys, indent=2))
