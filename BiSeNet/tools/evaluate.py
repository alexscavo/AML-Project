...
def eval_model(cfg, net):
    ...
    # size preprocessor (if you have any resizing rules in cfg)
    ...
    # single-scale, no-flip evaluator
    evaluator = MscEvalV0(
    ...
    # run evaluation
    metrics = evaluator(net, dl)

    # build IoU results table (single row)
    iou_heads = ['method', 'ratio', 'miou', 'fw_miou']
    iou_row   = [
        'ss',                               # method name
        '-',                                # ratio (not used here)
        f"{metrics['miou']:.6f}",          # mean IoU
        f"{metrics['fw_miou']:.6f}"        # frequency-weighted IoU
    ]
    iou_content = [iou_row]

    # build F1 results table (single row)
    f1_heads = ['method', 'ratio', 'macro_f1', 'micro_f1']
    f1_row   = [
        'ss',                               # method name
        '-',                                # ratio
        f"{metrics['macro_f1']:.6f}",      # macro F1
        f"{metrics['micro_f1']:.6f}"       # micro F1
    ]
    f1_content = [f1_row]

    # restore original aux setting
    net.aux_mode = org_aux

    #return iou_heads, iou_content, f1_heads, f1_content
    return iou_heads, iou_content, f1_heads, f1_content, metrics
...
def evaluate(cfg, weight_pth):
    ...
    iou_heads, iou_content, f1_heads, f1_content, metrics = eval_model(cfg, net)
    ...
    # --- NEW: per‐class IoU table ---
    # (make sure these names match your dataset’s trainIds 0–6)
    class_names = [
        'background',
        'building',
        'road',
    ...
    per_ious = metrics['ious']     # list of length n_cats
    per_table = [
        [cls, f"{iou:.6f}"]
        for cls, iou in zip(class_names, per_ious)
    ]
    logger.info('\nPer-class IoU:\n' +
        tabulate(per_table, headers=['class','IoU'], tablefmt='orgtbl'))
