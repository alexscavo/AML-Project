...
def unnormalize(img, mean, std):
    """
    img: Tensor with shape (C, H, W)
    mean, std: lists of means and stds for each channel
    """
...
if __name__ == '__main__':
    ...
    train_dataset = eval('datasets.'+config.DATASET.DATASET)(
        root=config.DATASET.ROOT, list_path=config.DATASET.TRAIN_SET,
    ...
    trainloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, shuffle=config.TRAIN.SHUFFLE,
